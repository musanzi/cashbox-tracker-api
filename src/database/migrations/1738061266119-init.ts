...
export class Init1738061266119 implements MigrationInterface {
  name = 'Init1738061266119';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`transaction\` (\`id\` varchar(36) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` datetime(6) NULL, \`amount\` decimal(10,2) NOT NULL DEFAULT '0.00', \`type\` enum ('deposit', 'withdrawal', 'transfer') NOT NULL, \`fromId\` varchar(36) NULL, \`toId\` varchar(36) NULL, \`byId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
    );
    await queryRunner.query(
